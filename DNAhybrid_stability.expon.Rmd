---
title: "DNAhybrid_stability.expon"
...
sdSW<-apply(SW, 2, sd)
SWdf<-data.frame(pos= -14:50, mean = meanSW[37:101], sd = sdSW[37:101])
...
hm  
ggplot2::ggsave("heatmap.DNAstabVsEI.mergedRepl.expon.pdf", plot = hm, device = "pdf", path = NULL,
...
#save the plot
plt$savefig("heatmap.InrStab.EI.expon.pdf")
#display the plot
plt$show()
```
...
hm <- ggplot(data=TPmelt, aes(x=pos, y=ratio)) + geom_raster(aes(fill=SpearmanR), interpolate = F) + 
   scale_fill_continuous_diverging("Vik", limits=c(-1,1), rev=T) + theme_void()
hm  
ggplot2::ggsave("heatmap.DNAstabVsTECprogress.mergedRepl.expon.pdf", plot = hm, device = "pdf", path = NULL,
       scale = 1, width = 90, height = 12, units = c("mm"), limitsize = TRUE)

write.table(TPmelt, "DNAhybrid.vs.TECprogress.expon.txt", col.names = T, row.names = F, quote=F)
...
#save the plot
plt$savefig("heatmap.InrStab.ratios.expon.pdf")
#display the plot
plt$show()
```
...
#save the plot
plt$savefig("heatmap.TATAscore.ratios.expon.pdf")
#display the plot
plt$show()
```
